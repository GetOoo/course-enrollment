...
export const PaperContainer = () => {
  const [courses, setCourses]: [any, any] = useState([]);
  const [pendedCourses, setPendedCourses]: [any, any] = useState([]);
  ...
  const subscribeHandler = (course: any) => {
  ...
  return (
    <Box className={ classes.Box }>
      <Paper
        ...
        elevation={ 20 }
      >
        { courses.slice(pageInfo.fromIndex, pageInfo.toIndex).map((course: { id: any }) =>
          <CourseCard
